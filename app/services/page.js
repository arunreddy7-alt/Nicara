  ...
  return (
    <>
      <main className="min-h-screen bg-white px-6 pt-10 pb-20 flex flex-col items-center">
        <h1 className="text-[6rem] font-light leading-none mb-18 text-black w-full text-left" style={{ fontFamily: 'Didot, serif' }}>Services</h1>
        <div className="w-full max-w-8xl grid grid-cols-1 md:grid-cols-3 gap-7">
          {services.map((service, idx) => (
            <div key={idx} className="flex flex-col items-center">
              <div className="w-full aspect-[4/5] relative mb-9">
                <Image
                  src={service.img}
                  alt={service.title}
                  fill
              ...
              <div className="text-6xl md:text-4xl font-light text-black text-left w-full" style={{ fontFamily: 'Didot, serif' }}>{service.title}</div>
              <p className="text-xl font-normal text-black text-left w-full mt-9" style={{ fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>
                {service.description}
              </p>
            </div>
          ))}
