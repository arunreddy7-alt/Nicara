  ...
  return (
    <nav
      className={`w-full border-b border-black/60 fixed top-0 z-50 transition-colors duration-300
        ${scrolledUp ? 'bg-white shadow-md' : 'bg-transparent'}
        px-4 sm:px-14 pt-2 sm:pt-4 pb-2 font-["Montserrat","Inter","Geist","sans-serif"]`}
    >
      {/* Desktop Layout */}
      <div className={`w-full hidden sm:flex ${scrolledUp ? 'flex-row items-center justify-between' : 'flex-col items-center justify-center'} transition-all duration-300`}>
        {/* Logo */}
        <div className={scrolledUp ? 'flex items-center z-10' : 'flex flex-col items-center z-10'}>
          <Link href="/">
            <Image src="/nicara-logo.png" alt="Nicara Logo" width={100} height={80} priority className="mb-2" />
          </Link>
        </div>
        {/* Nav Links */}
        <div className={scrolledUp ? 'flex flex-row gap-8 items-center justify-center flex-1 z-0' : 'flex flex-row gap-8 items-center justify-center mt-3 z-0'}>
